...
def parse_financials(api_key, rcept_no, diag) -> dict:
    ...
    if cash_bd:
        diag.append("  현금성자산 채택: " + " + ".join(f"{k}({v/1e8:.0f}억)" for k, v in cash_bd.items())
                    + f" = {sum(cash_bd.values())/1e8:.0f}억")
    if debt_bd:
        diag.append("  총차입금 채택: " + " + ".join(f"{k}({v/1e8:.0f}억)" for k, v in debt_bd.items())
                    + f" = {sum(debt_bd.values())/1e8:.0f}억")
    out["cash_bd"] = cash_bd
    out["debt_bd"] = debt_bd
